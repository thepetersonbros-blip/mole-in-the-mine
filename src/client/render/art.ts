...
export function drawTile(ctx: CanvasRenderingContext2D, t: number, m: number, tx: number, ty: number): void {
  const x = tx * P;
  const y = ty * P;
  if (t === T.AIR) return;
...
export function drawCamp(ctx: CanvasRenderingContext2D, cartGold: number, quota: number): void {
...
export function drawSky(ctx: CanvasRenderingContext2D, x0: number, x1: number): void {
  const top = 0;
  const h = SURFACE_Y * P;
  const grad = ctx.createLinearGradient(0, top, 0, h);
  grad.addColorStop(0, '#0c1226');
  grad.addColorStop(1, '#27314e');
  ctx.fillStyle = grad;
  ...
  for (let tx = tx0; tx <= tx1; tx++) {
    for (let ty = 0; ty < SURFACE_Y - 1; ty++) {
      if (hash2d(5, tx, ty) > 0.86) {
        ctx.globalAlpha = 0.3 + hash2d(6, tx, ty) * 0.7;
